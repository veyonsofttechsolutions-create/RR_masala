...
export default function Cart() {
  ...
  const handleQty = async (product, newQty) => {
    const id = product._id || product.id;
    setLoadingId(id);
    try {
      await setQty(product, newQty);
    } catch (error) {
      console.error(error);
    } finally {
      setLoadingId(null);
    ...
    <main className="premiumCartPage">
          ...
          <section className="cartItemsPanel">
            ...
            <div className="premiumCartItems">
              {items.map((i) => {
                ...
                const lineTotal = Number(product.price || 0) * i.quantity;
                const isBusy = loadingId === productId;
                    ...
                    <div className="cartQuantity">
                      ...
                      <div className="qtyControl">
                        <button
                          ...
                          disabled={isBusy || i.quantity <= 1}
                          style={{ opacity: (isBusy || i.quantity <= 1) ? 0.35 : 1 }}
                          onClick={() => handleQty(product, i.quantity - 1)}
                        >
                          <Minus size={14} />
                        </button>
                        <b>{i.quantity}</b>
      ...
      <style>
        {`
          ...
          .removeCartItem:hover:not(:disabled) {
            text-decoration: underline;
          }
          .removeCartItem:disabled {
            opacity: 0.5;
            cursor: not-allowed;
          }
          .shippingProgress {
